package kr.ac.kopo.ctc.kopo20.board.aop;
...
public class LogAspect {
	...
	@Before("execution(* kr.ac.kopo.ctc.kopo20.board.service.*.*AopBefore(..))")
	public void onBeforeHandler() {
		System.out.println("LogAspect.onBeforeHandler() 핸들러 호출");
	}
	
	@After("execution(* kr.ac.kopo.ctc.kopo20.board.service.*.*AopAfter(..))")
	public void onAfterHandler() {
		System.out.println("LogAspect.onAfterHandler() 핸들러 호출");
	}
	
	@AfterReturning(value="execution(* kr.ac.kopo.ctc.kopo20.board.service.*.*AopAfterReturning(..))", returning="returnValue")
	...
	@AfterThrowing(value="execution(* kr.ac.kopo.ctc.kopo20.board.service.*.*AopAfterThrowing(..))", throwing="exception")
	...
	@Around("execution(* kr.ac.kopo.ctc.kopo20.board.service.*.*AopAround(..))")
